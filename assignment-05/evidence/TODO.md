...
### Not milestone-blocking — a HARD RULE debt opened deliberately

**74. The root `README.md` diagram does not show the `goal-planner`.** · **KIND A**
Opened 2026-08-03. `CLAUDE.md`'s HARD RULE says a pipeline change is **not finished** until
**both** diagrams match reality, and adding an agent is a pipeline change. `CLAUDE.md` was
updated; the root `README.md` was **not**, because the Assignment 05 instruction said
explicitly not to touch it. **This is a knowingly-accepted debt, not an oversight.** Until it
is cleared the two diagrams disagree, and the HARD RULE's literal remedy is to treat every
gate as closed and dispatch nobody. Clearing it is a five-minute mirror of the two blocks
added to `CLAUDE.md`.
